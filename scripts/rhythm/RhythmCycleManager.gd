...
func _on_command_recognized(command: CharacterCommand, sequence: Array, time_since_beat: float) -> void:
	...
	if current_phase == CyclePhase.WAITING:
		# 第一次WAITING→IDLE切换，不跳过任何beat
		_switch_to_idle()
		# 手动emit idle_phase_started，让当前beat成为idle beat 1
		idle_phase_started.emit()
		# 不设置just_switched_to_idle，让下一个beat正常计数
		just_switched_to_idle = false
		print("🎵 首次命令识别（time=%.2fs），切换到空档期！" % time_since_beat)
...
func _switch_to_idle() -> void:
	print("🔧 _switch_to_idle()执行: beats_in_current_phase=%d -> 0" % beats_in_current_phase)
...
func _on_beat(beat: int) -> void:
	...
	if current_phase == CyclePhase.WAITING:
		if pending_switch_to_idle:
			# 延迟到await之后处理，避免在IDLE block之前切换
			pass
		else:
	...
	if rhythm_detector:
		...
		if pending_switch_to_idle and not idle_block_ran:
			...
			# 手动设置just_switched_to_idle=true，这样下一个beat会跳过IDLE block
			# 但当前beat（调用_switch_to_idle的beat）的END之后已经emit了，所以只跳过一个beat
